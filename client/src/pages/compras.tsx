...
export default function Compras() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Compra registada com sucesso" });
  ...
  const filtered = purchases.filter((p) =>
    !searchTerm || p.supplierName?.toLowerCase().includes(searchTerm.toLowerCase()) || p.number.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogTrigger asChild>
            <Button data-testid="button-new-purchase"><Plus className="w-4 h-4 mr-2" />Nova Compra</Button>
          </DialogTrigger>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Registar Compra</DialogTitle></DialogHeader>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  <TableRow key={p.id} data-testid={`row-purchase-${p.id}`}>
                    ...
                    <TableCell><Badge variant="secondary">{p.status === "paga" ? "Paga" : "Registada"}</Badge></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
